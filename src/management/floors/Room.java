...
public class Room {
	...
	public Room(String roomCode, int posX, int posY) {
		...
		for (int i = 1; i < height + 1; i++) {
			...
			for (int j = 0; j < splittedline.length; j++)
				this.tiles[width * (i - 1) + j] = new Tile(StringMatcher.getAbsoluteContent(splittedline[j]));
		}
		try {
			String[] ovtxts = splittedcode[1].split(";");
			overtextures = new Overtexture[ovtxts.length];
			for (int i = 0; i < ovtxts.length; i++) {
				String[] single = StringMatcher.getAbsoluteContent(StringMatcher.getBracketsContent(ovtxts[i], 1))
						.split(",");
				overtextures[i] = new Overtexture(
						new AnimatedSprite(new BufferedImage[] { Res_Tiles.tilessprites[Integer.parseInt(single[2])] }),
						Integer.parseInt(single[0]), Integer.parseInt(single[1]));
			}
		} catch (Exception e) {
	...
	/**
	 * Prints the room at the wanted coordinates. The x and y coordinates define the
	...
	public void print(Graphics2D g2d, int x, int y) {
		for (int i = 0; i < this.width; i++)
			for (int j = 0; j < this.height; j++)
				try {
					g2d.drawImage(getTile(i, j).sprite.get(), 16 * i + x, 16 * j + y, null);
					if (PlayerInfo.DEBUGMODE && getTile(i, j).type != Tile.TYPE_NORMAL) {
						g2d.setColor(new Color(255, 0, 0, 80));
						g2d.fillRect(16 * i + x, 16 * j + y, 16, 16);
					}
				} catch (Exception e) {
					System.err.println("Tile not printed in the current room at coordinates : " + i + " / " + j);
				}
	...
	/**
	 * Prints the overtexture of this room. The x and x coordinates define the top
	 * corner of the room. This is basically the same as the
	...
	public void printOvertexture(Graphics2D g2d, int x, int y) {
		for (int i = 0; i < overtextures.length; i++)
			try {
				g2d.drawImage(overtextures[i].sprite.get(), 16 * overtextures[i].x + x, 16 * overtextures[i].y + y,
						null);
			} catch (Exception e) {
	...
	public void deleteEntity(Entity pointer) {
		...
		if (removeid == -1)
			...
			System.arraycopy(entities, removeid + 1, newentities, removeid, entities.length - 1 - removeid);
