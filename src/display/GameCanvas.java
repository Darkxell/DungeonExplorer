...
public class GameCanvas extends Canvas {
	...
	public GameCanvas() {
		this.addMouseListener(new MouseListener() {
			...
			public void mouseReleased(MouseEvent e) {
				if (isMouseInside && mouseY < 20 && mouseX > getWidth() - 29)
					System.exit(0);
				if (isMouseInside && mouseY < 20 && mouseX > getWidth() - 58 && mouseX < getWidth() - 29)
					DungeonExplorer.frame.minimize();
				if (isMouseInside && mouseY < 20 && mouseX < 29 && !(state instanceof ControlState))
		...
		this.addMouseMotionListener(new MouseMotionListener() {
			...
			public void mouseDragged(MouseEvent e) {
				if (mouseY < 20)
					DungeonExplorer.frame.moveTo(e.getXOnScreen() - mouseX, e.getYOnScreen() - mouseY);
				if (isMouseOnCorner) {
					DungeonExplorer.frame.changeSize((e.getX() + 10 > 240) ? e.getX() + 10 : 240,
	...
	public void update(Graphics g) {
		if ((mouseY > getHeight() - 20 && mouseX > getWidth() - 20) || this.isMouseOnCorner) {
			if (!isCursorExtends) {
		...
		gr.drawImage(
				(isMouseInside && mouseY < 20 && mouseX > getWidth() - 29) ? Res_Frame.close_active : Res_Frame.close,
				getWidth() - 29, 0, null);
		gr.drawImage((isMouseInside && mouseY < 20 && mouseX < 29) ? Res_Frame.controls_active : Res_Frame.controls, 0,
				0, null);
		gr.drawImage((isMouseInside && mouseY < 20 && mouseX > getWidth() - 58 && mouseX < getWidth() - 29)
				? Res_Frame.minimize_active
				: Res_Frame.minimize, getWidth() - 58, 0, null);
		gr.setColor(Color.GRAY);
		if (PlayerInfo.DEBUGMODE)
			gr.drawString("Zelda : the minish cap | Debug mode enabled", 35, 15);
		else
			gr.drawString("Zelda : the minish cap", 35, 15);
		BufferedImage doublebuffer = new BufferedImage(ScreenWidth, ScreenHeight, BufferedImage.TYPE_INT_RGB);
