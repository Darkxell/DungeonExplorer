...
public class PlayerSpriteSheet extends SpriteSheet {
	...
	public PlayerSpriteSheet() {
		super(null);
		super.sprites = new AnimatedSprite[] { new AnimatedSprite(new BufferedImage[] { Res_Player.still_down }),
				new AnimatedSprite(new BufferedImage[] { Res_Player.still_left }),
				new AnimatedSprite(new BufferedImage[] { Res_Player.still_up }),
				new AnimatedSprite(Res_Player.walk_down), new AnimatedSprite(Res_Player.walk_left),
	...
	/**
	 * Returns the amount of frames the current image is supposed to stay, only
	 * according to the current AnimatedSprite ID being displayed. If the value is
	 * not specified, returns 1.
	 */
